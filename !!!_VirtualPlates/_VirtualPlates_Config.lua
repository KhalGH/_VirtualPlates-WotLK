...
local L = _VirtualPlates.L or {};
local PlatesVisible = _VirtualPlates.PlatesVisible
local me = CreateFrame( "Frame", "_VirtualPlatesConfig" );
_VirtualPlates.Config = me;

me.ScaleNormDist = CreateFrame( "Slider", "$parentScaleNormDist", me, "OptionsSliderTemplate" );
...
me.MaxScaleMax = 3;
me.ScaleNormDistMin =  5;
me.ScaleNormDistMax = (C_NamePlate and C_NamePlate.GetNamePlatesDistance) and math.min(tonumber(GetCVar("nameplateDistance")) or 41, 100) or 80
...
SubText:SetPoint( "TOPLEFT", Title, "BOTTOMLEFT", 0, -18 );
SubText:SetPoint( "RIGHT", -32, 0 );
SubText:SetHeight( 60 ); --32
SubText:SetJustifyH( "LEFT" );
SubText:SetJustifyV( "TOP" );
SubText:SetText( L.CONFIG_DESC );
SubText:SetFont("Fonts\\FRIZQT__.TTF", 11.5);

--- Creates a common slider with value text.
-- @ param Format  Number format used for min, max, and current value text.
local function SetupSlider ( self, Format, isMaxScale )
	self.Format = Format;
	self:SetPoint( "RIGHT", -20, 0 );
	self:SetScript( "OnValueChanged", me.SliderOnValueChanged );
	if isMaxScale then
		self:HookScript("OnValueChanged", function()
			if not _VirtualPlates.OptionsCharacter.DynamicScaling then
				for Plate, _ in pairs(PlatesVisible) do
					_VirtualPlates.PlateOnShow(Plate)
				end
			end
		end)
	end
...
SetupSlider( me.ScaleNormDist, L.CONFIG_SLIDERYARD_FORMAT ):SetPoint( "TOPLEFT", SubText, "BOTTOMLEFT", 6, -28 );
me.ScaleNormDist.Update = _VirtualPlates.SetScaleNormDist;
_G[ me.ScaleNormDist:GetName().."Text" ]:SetText( L.CONFIG_SCALENORMDIST );
me.ScaleNormDist.tooltipText = L.CONFIG_SCALENORMDIST_DESC;
me.SliderSetRange( me.ScaleNormDist, me.ScaleNormDistMin, me.ScaleNormDistMax );

-- Size limit options section
LimitsOptions:SetPoint( "TOPLEFT", me.ScaleNormDist, "BOTTOMLEFT", -8, -38 );
LimitsOptions:SetPoint( "RIGHT", -14, 0 );
_G[ LimitsOptions:GetName().."Title" ]:SetText( L.CONFIG_LIMITS );
_G[ LimitsOptions:GetName().."Title" ]:SetFont("Fonts\\FRIZQT__.TTF", 12)

SetupSlider( me.MinScale, L.CONFIG_SLIDER_FORMAT ):SetPoint( "TOPLEFT", 16, -32 );
...
me.SliderSetRange( me.MinScale, 0.1, 1 );
...
SetupSlider( me.MaxScale, L.CONFIG_SLIDER_FORMAT, true ):SetPoint( "TOPLEFT", me.MaxScaleEnabled, "BOTTOMLEFT", 4, -8 );
me.MaxScale.Update = _VirtualPlates.SetMaxScale;
_G[ me.MaxScale:GetName().."Text" ]:SetText( L.CONFIG_MAXSCALE );

LimitsOptions:SetHeight( me.MinScale:GetHeight() + me.MaxScaleEnabled:GetHeight() + me.MaxScale:GetHeight() + 100 );
