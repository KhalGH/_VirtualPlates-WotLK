...
local totemGlowSize = 128 * totemSize / 88
...
do
	...
	function me:PlateOnShow ()
		...
		local nameText = select(7, Virtual:GetRegions()):GetText()
		local totemTex = TotemPlates[nameText]
		if totemTex then
			if not self.totemPlate then
				-- Setup TotemPlate on the fly
				self.totemPlate = CreateFrame("Frame", nil, self)
				self.totemPlate:SetPoint("CENTER", Virtual, 0, totemOffSet)
				self.totemPlate:SetSize(totemSize, totemSize)
				self.totemPlate:Hide()
				self.totemPlate.icon = self.totemPlate:CreateTexture(nil, "ARTWORK")
				self.totemPlate.icon:SetAllPoints(self.totemPlate)
				self.totemPlate.targetGlow = self.totemPlate:CreateTexture(nil, "OVERLAY")
				self.totemPlate.targetGlow:SetTexture(texturePath .. "TotemPlate-TargetGlow.blp")
				self.totemPlate.targetGlow:SetPoint("CENTER")
				self.totemPlate.targetGlow:SetSize(totemGlowSize, totemGlowSize)
				self.totemPlate.targetGlow:Hide()
				self.totemPlate.mouseoverGlow = self.totemPlate:CreateTexture(nil, "OVERLAY")
				self.totemPlate.mouseoverGlow:SetTexture(texturePath .. "TotemPlate-MouseoverGlow.blp")
				self.totemPlate.mouseoverGlow:SetPoint("CENTER")
				self.totemPlate.mouseoverGlow:SetSize(totemGlowSize, totemGlowSize)
				self.totemPlate.mouseoverGlow:Hide()
				self.totemDelay = CreateFrame("Frame")
			end
			Virtual:Hide()
			-- Delay frame to ensure namePlateUnitToken is available
			if C_NamePlate then
				self.totemDelay:SetScript("OnUpdate", function(totemDelay)
					totemDelay:SetScript("OnUpdate", nil)
					...
					if unitToken then
						...
						if totemTex ~= "" and isHostile then
							...
							self.totemPlate.icon:SetTexture(totemTex)
						end	
					end
				end)
			...
			local levelText = select(8, Virtual:GetRegions()):GetText()
			...
			elseif C_NamePlate then
				-- Delay frame to ensure namePlateUnitToken is available
				self.filterUnitsDelay = self.filterUnitsDelay or CreateFrame("Frame")
				self.filterUnitsDelay:SetScript("OnUpdate", function(filterUnitsDelay)
					filterUnitsDelay:SetScript("OnUpdate", nil)
					local unitToken = self.namePlateUnitToken
					if unitToken then
						...
						if filterPlayers and not isPlayer then
							...
							if Virtual.classIcon and isPlayer and isFriendly and inPvPinstance and ShowClassIcons then
								local class = select(2, UnitClass(unitToken))
								Virtual.classIcon:SetTexture(texturePath .. "Class\\" .. class)
				...
				if ( #SortOrder > 0 ) then
					...
					for Index, Plate in ipairs( SortOrder ) do
						...
						if C_NamePlate_GetNamePlatesDistance then 
							...
							if Distance then
								...
								if Distance <= fadeEnd or UnitIsPlayer(unitToken) == 1 then
									local alpha = math.max(0, math.min(1, 1 - ((Distance - fadeStart) / (fadeEnd - fadeStart))))
									SetRegionsAlpha(alpha)
									SetAlpha(Virtual, 1)
								else
									local alpha = math.max(0, math.min(1, 1 - ((Distance - fadeEnd) / 30)))
									SetRegionsAlpha(0)
						...
						if totemTex then
							...
							-- Shrinks the nameplate hitbox to effectively disable interaction
							Scale = 0.001
						end
						...
						if not Virtual:IsShown() then Scale = 0.001 end -- by Khal
...
SlashCmdList["VPICONS"] = function()
	if C_NamePlate then
		...
		for _, plate in pairs(RealPlates) do
			local Virtual = VirtualPlates[plate]
			if Virtual then
				local unitToken = plate.namePlateUnitToken
				local isPlayer = unitToken and UnitIsPlayer(unitToken) == 1
				local reaction = UnitReaction("player", unitToken)
				local isFriendly = reaction and reaction >= 5
				local ShowClassIcons = me.OptionsCharacter.ShowClassIcons
				if Virtual.classIcon and isPlayer and isFriendly and inPvPinstance and ShowClassIcons then
	...
	if C_NamePlate then
		...
		for _, plate in pairs(RealPlates) do
			...
			if Virtual then
				...
				local levelText = select(8, Virtual:GetRegions()):GetText()
				local levelNumber = tonumber(levelText)
				local meetsMinLevel = levelNumber and levelNumber >= nameplateMinLevel
				if filterPlayers and not isPlayer then
					Virtual:Hide()
				elseif meetsMinLevel then
					Virtual:Show()
				end
			end
		end
